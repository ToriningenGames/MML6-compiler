...
a batch file to automate the process on Windows (sorry Linux people!),
...
Songwriter/Player Use:
 - Write your song! A few files are provided in the Songs subdirectory to start with
	 ...
	 - The songs in the Songs subdirectory are two test bits (Test and scale), and two Touhou adaptations (Spark and Doll).
 ...
 - Move Song.mcs to the musPlayer subdirectory.
 - Assemble musplayer.asm. It is written for WLA-GB, and a copy is provided if you have none.
 - Link together the ROM. The linkfile should look like this:
	[objects]
	your_object_file_here.obj
...
Game development Use:
 ...
 - Instructions for assembly use are in Sound.asm
...
Q&A:
Q: I just want stuff to work! (Windows x64)
A: Okay, dump everything in the same directory, and open a song file with musPlayer.bat.

Q: Stuff doesn't run! (Windows x86)
A: The provided binaries are for Windows x64. You'll need a 32 bit copy of WLA-GB and GCC to use everything.
...
A: Well, the output song files are small, and are designed to be played alongside a game.
	...
	Therefore, this is an extensible and portable option for music-writing on a Gameboy, and your pieces can be put in games!

Q: Why is it called "MML6"? Where are the other five?
A: Because when I started, I knew nothing of planning, and went through 6 different approaches to making the format. I feel the result is decent. The others... not so much.
...
A: It's designed for GCC on Windows via Mingw. It _should_ work on any system, except the clean target requires redefining DEL towards a file remover.
